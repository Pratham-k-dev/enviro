...
// ─── Extra Environment Components (fully hardcoded, unchanged) ─────────────────
const EXTRA_ENV = [
  {
    icon: <CloudRain size={22} />,
    title: "Precipitation",
...
// ─── Chemical Hazards (source + level hardcoded; impact = AI) ─────────────────
const CHEM_BASE = [
  {
    key: "pm25",
    name: "PM₂.₅",
...
// ─── Activity Window Data (hardcoded, unchanged) ───────────────────────────────
const ACTIVITY_WINDOWS = [
  {
    time: "06:00 – 09:00",
    label: "Morning",
...
// ─── AI Fetch Helper ──────────────────────────────────────────────────────────
async function fetchAIInsight(type, env, conditions) {
  try {
    const res = await fetch("/api/ai-insight", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type, env, conditions }),
    })
...
export default function Dashboard() {
  ...
  // AI state for the two sections
  const [metricsAI, setMetricsAI] = useState(null)   // ENV_META explainers (per-metric)
  const [hazardsAI, setHazardsAI] = useState(null)   // ChemCard biological impact
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // ── Fire AI calls after env data is available ──
        const envPayload = {
          location: json.env?.location,
          aqi: json.env?.air?.aqi,
          pm25: json.env?.air?.pm25,
          pm10: json.env?.air?.pm10,
          so2: json.env?.air?.so2,
          no2: json.env?.air?.no2,
          co: json.env?.air?.co,
          temp: json.env?.weather?.temp,
          humidity: json.env?.weather?.humidity,
          wind: json.env?.weather?.wind,
          uv: json.env?.uv?.index,
          condition: json.env?.weather?.condition,
        }

        const userConditions = []
        if (json.profile?.asthma) userConditions.push("asthma")
        if (json.profile?.dust_allergy) userConditions.push("dust allergy")
        if (json.profile?.pollen_allergy) userConditions.push("pollen allergy")

        setAiLoading(true)
        const [metricsText, hazardsText] = await Promise.all([
          fetchAIInsight("metrics", envPayload, userConditions),
          fetchAIInsight("hazards", envPayload, userConditions),
        ])

        console.log(metricsText , hazardsText)
        setMetricsAI(metricsText)
        setHazardsAI(hazardsText)
        setAiLoading(false)
      } catch (e) {
        console.error(e)
        setLoading(false)
  ...
  // Filter insights based on profile conditions (unchanged)
  const visibleInsights = HARDCODED_INSIGHTS.filter(ins => {
    if (!ins.condition) return true
    if (ins.condition === 'asthma' && profile?.asthma) return true
    if (ins.condition === 'dust_allergy' && profile?.dust_allergy) return true
  ...
  // Build chemicals array with real values from env
  const chemicals = CHEM_BASE.map(c => ({
    ...c,
    value: env?.air?.[c.key] ?? "—",
  }))
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9] text-slate-900 py-8 px-4 relative overflow-hidden">
      ...
      >
        ...
        {/* ── Core Env Metrics — "What this means" powered by AI ─────────── */}
        <div>
          <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
            <Activity className="text-sky-500" /> Environmental Metrics
          </h3>

          {/* ── AI Metrics Summary Banner ── */}
          <div className="mb-5 p-4 bg-white rounded-2xl border border-sky-100 shadow-sm flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-[10px] font-black text-sky-500 uppercase tracking-widest mb-1">What This Means — AI Analysis</p>
              {aiLoading ? (
                <div className="flex items-center gap-2">
                  <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
                    className="w-3 h-3 border-2 border-sky-400 border-t-transparent rounded-full" />
                  <p className="text-[11px] text-slate-400 font-medium">Analysing your environment...</p>
                </div>
              ) : metricsAI ? (
                <p className="text-[11px] text-slate-700 leading-relaxed font-medium">{metricsAI}</p>
              ) : (
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  AQI, UV, temperature, and humidity readings combined determine your real-time exposure risk.
                  These values are cross-referenced with your health profile to surface personalised alerts.
                </p>
              )}
            </div>
          </div>
        ...
        {/* ── Extra Env Components (hardcoded, unchanged) ─────────────────── */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {EXTRA_ENV.map((e, i) => (
            <ExtraEnvCard key={i} item={e} />
          ))}
        ...
        {/* ── Chemical Hazard Analysis — Biological Impact powered by AI ─── */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              <Microscope className="text-indigo-500" /> Chemical Hazard Analysis
            </h3>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-white px-3 py-1 rounded-full border border-slate-100 shadow-sm">Live Lab Scan</span>
          </div>

          {/* ── AI Hazards Summary Banner ── */}
          <div className="p-4 bg-white rounded-2xl border border-rose-100 shadow-sm flex items-start gap-3">
            <div className="w-8 h-8 bg-rose-50 rounded-xl flex items-center justify-center shrink-0 mt-0.5">
              <ShieldAlert size={15} className="text-rose-500" />
            </div>
            <div className="flex-1">
              <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-1">Biological Impact — AI Analysis</p>
              {aiLoading ? (
                <div className="flex items-center gap-2">
                  <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
                    className="w-3 h-3 border-2 border-rose-400 border-t-transparent rounded-full" />
                  <p className="text-[11px] text-slate-400 font-medium">Analysing chemical exposure...</p>
                </div>
              ) : hazardsAI ? (
                <p className="text-[11px] text-slate-700 leading-relaxed font-medium">{hazardsAI}</p>
              ) : (
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Chemical readings are evaluated against WHO safety thresholds. Exposure impact
                  varies by individual health sensitivity and cumulative pollutant interaction.
                </p>
              )}
            </div>
          </div>
        ...
        {/* ── Activity Windows (hardcoded, unchanged) ─────────────────────── */}
        <div className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            <Clock size={18} className="text-slate-500" />
            <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest">Activity Windows — Today's Schedule</h3>
        ...
        {/* ── Personalized Medical Analysis + Action Plan (hardcoded, unchanged) ── */}
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              <h3 className="text-xl font-bold mb-2 flex items-center gap-2"><ShieldCheck className="text-emerald-500" /> Personalized Medical Analysis</h3>
          ...
          {/* Action Plan (hardcoded, unchanged) */}
          <div className="bg-slate-900 rounded-[2.5rem] p-7 text-white flex flex-col justify-between">
            <div>
              <h3 className="text-xl font-bold mb-1 flex items-center gap-2 text-emerald-400"><Navigation size={20} /> Action Plan</h3>
              <p className="text-slate-500 text-xs mb-5 font-medium">Tailored for your profile & today's conditions</p>
...
// EnvInsightCard — "What this means" explainer removed (now in the AI banner above)
function EnvInsightCard({ icon, title, value, shortDesc, color }) {
  const colors = {
    sky: "bg-sky-50 text-sky-500 border-sky-100",
    yellow: "bg-yellow-50 text-yellow-500 border-yellow-100",
    orange: "bg-orange-50 text-orange-500 border-orange-100",
  ...
  return (
    <motion.div
      whileHover={{ scale: 1.02 }}
      className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col h-full"
    >
      <div className={`w-10 h-10 ${colors[color]} rounded-xl flex items-center justify-center mb-4 shrink-0`}>
        {icon}
      </div>
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{title}</p>
      <p className="text-2xl font-black mb-1">{value}</p>
      <p className="text-[11px] text-slate-500 leading-relaxed font-medium">{shortDesc}</p>
    </motion.div>
  )
}
